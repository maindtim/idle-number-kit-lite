...
- **`big_number.gd`** — `BigNumber`: stores a value as `mantissa * 10^exponent`
...
- **`number_formatter.gd`** — `NumberFormatter`: turns a `BigNumber` into
...
Copy `big_number.gd` and `number_formatter.gd` into your Godot 4 project.
